  ...
  <aside className="w-full lg:w-80 shrink-0">
    <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200 sticky top-24">
      ...
      <div className="space-y-6">
        ...
        <div>
          <Label className="font-semibold text-gray-600">BHK</Label>
          <Select
            value={filters.bhk}
            onValueChange={(v) => setFilters((p) => ({ ...p, bhk: v }))}
          >
            <SelectTrigger className="mt-2 bg-gray-100 border-transparent h-12">
              <SelectValue />
            </SelectTrigger>
...
const ThreeDPlansPage = () => {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    let products = combinedProducts.filter(
      (p) => p.planType === "3D Elevations"
    );
    ...
    products = products.filter((product) => {
      ...
      const matchesBhk =
        filters.bhk === "all" ||
        (product.rooms || product.bhk) === Number(filters.bhk);

      return matchesBudget && matchesPlotSize && matchesBhk;
    });
  ...
  const pageTitle = "3D Elevation Plans";
